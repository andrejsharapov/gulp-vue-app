...
Vue.component('site-skill-list', {
    props: [ 'skills' ],
    template: `<div id="skillsLanguage" v-for="(item, index) in skills" :key="index" v-dragscroll.x="true"><div class="skill-container" v-for="(i, index) in item.all" :key="index"><p class="lang_name">{{ i.langName }}</p><div class="skill-block"><img :src="'/src/skills/' + i.image" loading='lazy' :alt="i.langName"></div></div></div>`,
});
